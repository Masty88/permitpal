...
def cleanup_results_folder(results_folder="results"):
...
def create_cumulative_json(ifc_file, results_folder="results"):
...
def main(ifc_path):
    ...
    result1 = subprocess.run([sys.executable, "detect_space.py", ifc_file])
    if result1.returncode != 0:
        print("Error: detect_space.py did not complete successfully.")
        sys.exit(result1.returncode)

    print("Running extreme_x_y_z.py ...")
    result2 = subprocess.run([sys.executable, "extreme_x_y_z.py", ifc_file])
    ...
    result3 = subprocess.run([sys.executable, "get_ext_walls_id.py", ifc_file])
    ...
    cumulative_filename = create_cumulative_json(ifc_file, "results")
